...
class ServersIpsJsonTest(test_servers.ServersSampleBase):
    ...
    extension_name = 'ips'
    section_name = 'IPs'
    section_doc = 'Shows and lists IP information for server'

    def test_get(self):
        # Test getting a server's IP information.
        uuid = self._post_server()
        response = self._doc_do_get(
            'servers/%s/ips', uuid, 'server_id',
            api_desc="Gets a server's IP information.")
    ...
    def test_get_by_network(self):
        ...
        uuid = self._post_server()
        response = self._doc_do_get(
            'servers/%s/ips/%s', (uuid, 'private'),
            ('server_id', 'network_id'),
            api_desc="Gets a server's IP information by network id.")
        subs = self._get_regexes()
